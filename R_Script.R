#----------------------------------------------------------------#
#R_Programming Individual Project - 
#Exploratory Data Analysis and Data Visualization 
#by Paliz Mungkaladung - MBD 2021
#----------------------------------------------------------------#

#----------------------------------------------------------------#
#00 - Introduction
#----------------------------------------------------------------#
#We are going to perform EDA on the AMS 2013-2014 Solar Energy Prediction Contest 
...
#The real values of solar production recorded in 98 different weather stations 
#ranging from 1994-01-01 to 2012-11-30
solar_dataset <- readRDS(file = '~/R_Programming/solar_dataset.RData')

#The 100 original variables detected as more important to predict 
#the first station (column 2 ACME) values, after feature importance analysis 
...
#3.2 - Data cleaning
#----------------------------------------------------------------#
#Next, we starts to the data cleaning process. 
#Let's look for missing values using the DataExplorer package.

#plot_missing(solar_dataset, missing_only = TRUE)
options(repr.plot.width=8, repr.plot.height=3)
plot_missing(solar_dataset)

#Looking at the size of the dataset and the missing value plot, 
#it seems as if we can remove the missing values and still have 
#a good-sized set of data to work on, so let's start by doing that.
solar_dataset <- na.omit(solar_dataset)
dim(solar_dataset)

#New dimensions : 5113 rows and 456 columns

#Let's convert the column Date (character) to datetime. 
#It' will's going be useful for our EDA.
...
solar_dataset_dt <- tidyr::separate(solar_dataset, Date, 
            c('year', 'month', 'day'), sep = "-",remove = FALSE)

#Dropping the column Date
#solar_dataset_ymd <- subset(solar_dataset_dt, select = -c(1))

#We will also create a new variable that tells us the day of the week, 
#using the wday function from the lubridate package.
solar_dataset_dt$dayOfWeek <- wday(solar_dataset_dt$Date, label=TRUE)

#Because the Principal components won't be used in this study. 
#We will exclude all PC columns in order to consider only the real values 
#of solar production recorded in 98 different weather stations
sub_solar_dataset <- subset(solar_dataset_dt, select = -c(103:459))

dim(sub_solar_dataset)
head(sub_solar_dataset)
...
#----------------------------------------------------------------#
#Using diagnose_web_report to check out data. It generates the previous steps 
#of data diagnosis automatically. 
...
#If p-value =< alpha A , data isn't normalized.
#Fortunately, we are good here. 
...
#We can improve this part by extracting the columns date to datetime, day, month, and year. 
correlate(sub_solar_dataset)

#Plot correlation matrix 
plot_correlate(sub_solar_dataset)
#It seems like we have too many features. The plot isn't visible. 

#----------------------------------------------------------------#
#4.3 - Generateing EDA report
#----------------------------------------------------------------#
...
g <- list(
  scope = "usa",
  projection = list(type = "albers usa", scale = 1),
  showland = TRUE,
...
fig <- plot_geo(station_info, lat = ~nlat, lon = ~elon)
#fig <- plot_geo(station_info, lat = ~nlat, lon = ~elon)
fig <- fig %>% add_markers(
  text = ~paste(stid, paste("Elevation (m):", elev), sep = "<br />"),
  color = ~elev, symbol = I("circle"), size = I(20), hoverinfo = "text"
)
fig <- fig %>% colorbar(title = "Elevation (feet)")
fig <- fig %>% layout(
  title = "Station info.", geo = g
)
...
#6 - Conclusion
#----------------------------------------------------------------#
#We see that the solar_dataset has to many features (station). 
#This is a big challenge in performing EDA by R programming. 
#Becasue it requires a proper data preparation to make it easy to visualize. 
#I believe that working further of plotting day of week, month and year VS 
#sum of solar energy of each station will give us more insight of this dataset. 
